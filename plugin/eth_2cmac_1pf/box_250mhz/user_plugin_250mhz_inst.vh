...
// *************************************************************************
//
// Instantiates the pure-ethernet single-PF / N-CMAC plugin
// eth_2cmac_1pf_250mhz.  This module deliberately has 1 PF / N CMAC, so the
// stock NUM_PHYS_FUNC == NUM_CMAC_PORT assertion does NOT apply here and is
// intentionally omitted.
//
// Vivado 2024.2 requires $fatal's first argument to be a finish number
// (0/1/2); hence $fatal(1, ...).
//
// *************************************************************************
initial begin
  if (USE_PHYS_FUNC == 0) begin
    $fatal(1, "No implementation for USE_PHYS_FUNC = %d", 0);
  end
end
