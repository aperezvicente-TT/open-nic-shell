...
(ethtool `-x/-X`), per-port RSS spread (Ch. 11), **GRO** (`napi_gro_receive`),
and XDP.

**Advertised offloads:** `NETIF_F_HIGHDMA` only (+ default-on soft GRO). No
scatter-gather (HW-blocked — §10.2 1.3), no checksum, no segmentation, no VLAN
offload, no `ndo_set_rx_mode`. That is the starting point for everything below.
...
| 1.3 | **Scatter-gather TX** (`NETIF_F_SG`) ⚠️ *driver done, blocked by HW* | **Driver + FPGA** | S (driver) / M (FPGA) | Driver side is complete: `onic_xmit_frame` emits one QDMA H2C descriptor per skb fragment (SOP on the head, EOP on the last, `.len` = fragment size, `metadata[15:0]` = *total* packet length on every descriptor), `qdma_pack_h2c_st_desc` takes SOP/EOP from `desc->flags`, `onic_tx_clean` frees the skb only from the EOP slot. **HW testing (2026-07-14, au200 1PF/2CMAC, QSFP0↔ConnectX-6) hit a descriptor-encoding bug — NOT an IP gather limitation.** The QDMA H2C ST engine *does* support multi-descriptor packets (Xilinx reference DPDK builds one descriptor per fragment, SOP/EOP). The failure is OpenNIC's descriptor-field overload: the authoritative EQDMA5 descriptor is `cdh_flags`[15:0] \| `pld_len`[31:16] \| `len`[47:32] \| `flags`[63:48], but this driver folds `cdh_flags`+`pld_len` into a 32-bit `metadata` and repurposes `metadata[15:0]` as the shell's `tkeep` packet-length sideband. My SG code set `metadata=total_len` on the SOP descriptor → `cdh_flags`=bogus `NUM_GL`/`NUM_CDH` → the engine misparses desc0 and ends the packet early. On-wire: a 190B 2-descriptor frame arrived garbage-payloaded and 64B (one AXI beat) short; peer drops it (`frame_len != IP_len`). The driver's `len`/SOP/EOP were correct; the `metadata`↔`cdh_flags` overlap is the culprit. Linear TX is flawless (9.45 Gbit/s, 0 retr). A driver-only fix was **attempted and ruled out**: a 6-encoding sweep (legacy; Xilinx-reference `cdh_flags=0`/`pld_len=seg`; `pld_len=seg\|cdh=total`; `ZERO_CDH` variants; total-on-EOP-only) all failed identically. Real cause: **H2C ST runs in INTERNAL mode** (`qdma_subsystem.sv` ties off the H2C descriptor-bypass input), and internal mode ignores multi-descriptor SOP/EOP framing — SOP/EOP only matter in descriptor-bypass mode (cf. libqdma `descq_proc_st_h2c_request`). So each descriptor is its own packet regardless of what the driver writes. **`NETIF_F_SG` is therefore NOT advertised** (stack linearizes; 9.1–9.5 Gbit/s, 0 retr). Multi-descriptor code retained, dormant. **To unblock (FPGA work):** run H2C in descriptor-bypass mode with shell RTL driving `h2c_byp_in_st_{addr,len,sop,eop}` per fragment, or add a coalescing shim ahead of the CMAC. |
| 1.4 | **ethtool ring & coalesce control** (`get/set_ringparam`, `get/set_coalesce`) | Driver | S | Ops are currently absent (Ch. 5 §5.1 ethtool list). Standard tuning knobs. |
| 1.5 | **Pause / flow control** (`get/set_pauseparam`) | Driver (CMAC FC registers exist: `CONF_RX_FC_CTRL_1/2` @ `+0x0084/0x0088`) | S | Wire the existing CMAC flow-control CSRs to ethtool. |
| 1.6 | **Complete, correct statistics** (per-queue + HW counters through `get_stats64` / `-S`) | Driver + read CMAC/adapter counters | S–M | ethtool `-S` exists but should surface the CMAC FCS/error and `CMAC_ADPT` drop counters (`+0x3000` block, Ch. 9 §9.5) and the plugin diag counters (Ch. 4 §4.5). |
...
| 2.3 | **GRO** ✅ *already active* / LRO | Driver (GRO) / FPGA (LRO) | S (GRO) | **GRO already works:** `onic_rx_poll` delivers via `napi_gro_receive` (onic_netdev.c) and GRO is a default-on soft feature (`ethtool -k` shows `generic-receive-offload: on`). Nothing to implement. LRO would need a HW coalescing engine and remains open. |
